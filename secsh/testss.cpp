...
void SecretShareFile1(int threshold, int nShares, const char *words, const char *seed);
...
int CRYPTOPP_API main(int argc, char *argv[])
{
	...
	try
	{
		...
		else if (command == "testss")
		{
			char seed[1024];
			cout << "\nRandom Seed: ";
			ws(cin);
			cin.getline(seed, 1024);
			SecretShareFile1(atoi(argv[2]), atoi(argv[3]), argv[4], seed);
		}
		else
		{
...
void SecretShareFile1(int threshold, int nShares, const char *words, const char *seed)
{
	...
	RandomPool rng;
	rng.IncorporateEntropy((byte *)seed, strlen(seed));
	char* filename="secret";
	ChannelSwitch *channelSwitch;
	FileSource source(in, false, new SecretSharing(rng, threshold, nShares, channelSwitch = new ChannelSwitch));
