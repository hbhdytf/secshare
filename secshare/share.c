...
void share(char*fn)
{
	...
	/*generate random x*/
	mpz_init(x);
	//for(i=0;i<16;i++)rnd[i]=getfeaturekey();
	getfeaturekey(rnd,keyfile);
	setbin(x,rnd,16);
	...
	for(i=0;i<n;i++){
		/*k=k + kvec[i]*x^i*/
		mpz_powm(t,x,e,p);//t=x^e mod p
		mpz_mul(t,t,kvec[i]);//t=t*kvec[i]
		mpz_add(k,k,t);//k=k+t
		mpz_mod(k,k,p);//k=k mod p
...
void getfeaturekey(byte* buf,char* filename)
{
	FILE *fp;
	char str[40];
	fp=fopen(filename,"r");
